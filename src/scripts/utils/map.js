...
export default class Map {
  ...
  static async build(selector, options = {}) {
    ...
    const jakartaCoordinate = [-6.2, 106.816666]; // Using Geolocation API
  ...
  changeCamera(coordinate, zoomLevel = null) {
    if (!coordinate) {
      console.warn('changeCamera: coordinate is null or undefined');
      return;
    }

    // Kalau coordinate dikirim dalam bentuk object { lat, lon }
    let targetLatLng;
    if (Array.isArray(coordinate)) {
      targetLatLng = latLng(coordinate[0], coordinate[1]);
    } else if ('lat' in coordinate && ('lng' in coordinate || 'lon' in coordinate)) {
      targetLatLng = latLng(coordinate.lat, coordinate.lng ?? coordinate.lon);
    } else {
      console.error('changeCamera: invalid coordinate format', coordinate);
      return;
    }

    if (!zoomLevel) {
      this.#map.setView(targetLatLng, this.#zoom);
      return;
    }
