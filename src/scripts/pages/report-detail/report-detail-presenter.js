...
export default class ReportDetailPresenter {
  ...
  async showReportDetail() {
    ...
    try {
      ...
      console.log('showReportDetail: report =', report);
  ...
  async saveReport() {
    try {
      ...
      const response = await this.#apiModel.getReportById(this.#reportId);
      const mappedReport = await reportMapper(response.report || response.story || response.data);

      await this.#dbModel.putReport(mappedReport);
      this.#view.saveToBookmarkSuccessfully('Success to save to bookmark');
    } catch (error) {
